...
/* Product catalog follows the category order currently shown by EdTechHub:
   1) หลักสูตรภาษาอังกฤษสำหรับเด็ก
   2) หลักสูตรภาษาอังกฤษเพื่อการสื่อสาร
   3) สื่อมัลติมีเดียระดับปฐมวัย
   4) สื่อมัลติมีเดียระดับประถมศึกษา
   5) หนังสืออิเล็กทรอนิกส์ (e-Book)
   6) LED Interactive Intelligent Panel
   7) English Proficiency Test
*/
const PRODUCT_GROUPS=[
 {
  title:'หลักสูตรภาษาอังกฤษสำหรับเด็ก',
  en:'CHILDREN\'S ENGLISH PROGRAMS',
  intro:'หลักสูตรภาษาอังกฤษสำหรับเด็กที่เน้นการเรียนรู้แบบผสมผสาน สนุก และพัฒนาทักษะภาษาอย่างเป็นขั้นตอน',
  products:[
 ...
 {
  title:'หลักสูตรภาษาอังกฤษเพื่อการสื่อสาร',
  en:'COMMUNICATIVE ENGLISH PROGRAMS',
  intro:'หลักสูตรเพื่อฝึกการสื่อสารภาษาอังกฤษในสถานการณ์จริง พร้อมแนวทางการเรียนรู้ที่สอดคล้องกับ CEFR',
  products:[
 ...
 {
  title:'สื่อมัลติมีเดียระดับปฐมวัย',
  en:'PRESCHOOL MULTIMEDIA',
  intro:'สื่อการเรียนรู้ที่ออกแบบให้เหมาะกับเด็กปฐมวัยและการใช้งานผ่านห้องเรียนหรือกระดานอัจฉริยะ',
  products:[
 ...
 {
  title:'สื่อมัลติมีเดียระดับประถมศึกษา',
  en:'PRIMARY MULTIMEDIA',
  intro:'สื่อดิจิทัลเพื่อการเรียนการสอนระดับประถมศึกษา ครอบคลุมหลายกลุ่มสาระและใช้งานกับห้องเรียนสมัยใหม่',
  products:[
 ...
 {
  title:'หนังสืออิเล็กทรอนิกส์ (e-Book)',
  en:'E-BOOK & DIGITAL READING',
  intro:'หนังสืออิเล็กทรอนิกส์และกิจกรรมดิจิทัลที่ใช้งานได้ทั้งในห้องเรียนและการเรียนรู้ด้วยตนเอง',
  products:[
   {name:'eBook กิจกรรมพัฒนาทักษะเด็กปฐมวัย',desc:'e-Book แบบ 3 มิติ พร้อมกิจกรรมระบายสีและกิจกรรมพัฒนาทักษะสำหรับเด็กปฐมวัยจำนวนมาก',level:'e-Book',img:IMG+'product-p5.jpg',source:EDTECH+'17570280/หนังสืออิเล็กทรอนิกส์-e-book'}
  ]
 },
 {
  title:'LED Interactive Intelligent Panel',
  en:'SMART CLASSROOM DISPLAY',
  intro:'จอ Interactive สำหรับห้องเรียนและการประชุม พร้อมระบบสัมผัสและความละเอียดสูง',
  products:[
 ...
 },
 {
  ...
  intro:'เครื่องมือประเมินความสามารถทางภาษาอังกฤษเพื่อวัดระดับและใช้วางแผนพัฒนาผู้เรียน',
  products:[
...
function productCard(p,i){
 return `<article class="card product-card product-catalog-card">
   ...
   <div class="product-body"><h3>${esc(p.name)}</h3><p>${esc(p.desc)}</p><div class="product-actions"><button class="detail-btn" data-product="${i}">ดูรายละเอียด</button><a class="source-btn" href="${esc(p.source||'#')}" target="_blank" rel="noopener">ดูข้อมูลต้นทาง</a></div></div>
 </article>`;
}
...
function renderProducts(){
 ...
 el.innerHTML=PRODUCT_GROUPS.map((group,gi)=>{
   const cards=group.products.map(p=>{const flatIndex=FLAT_PRODUCTS.length;FLAT_PRODUCTS.push(p);return productCard(p,flatIndex);}).join('');
   return `<section class="product-category" data-product-category="${gi}"><div class="product-category-head"><div><span class="eyebrow">${esc(group.en)}</span><h3>${esc(group.title)}</h3><p>${esc(group.intro)}</p></div><span class="product-count">${group.products.length} รายการ</span></div><div class="product-category-grid">${cards}</div></section>`;
 }).join('');
...
function renderTraining(){
 ...
 el.innerHTML=TRAINING.map(t=>`<article class="card service-card">${imgTag(t.img,t.title)}<h3>${esc(t.title)}</h3><ul>${t.items.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></article>`).join('');
}
function openProduct(i){
 const p=FLAT_PRODUCTS[i], modal=document.getElementById('productModal');
 if(!p||!modal)return;
 const img=document.getElementById('modalImage');
 img.src=p.img; img.alt=p.name; img.onerror=()=>{img.onerror=null;img.src=IMG+'logo.png';};
